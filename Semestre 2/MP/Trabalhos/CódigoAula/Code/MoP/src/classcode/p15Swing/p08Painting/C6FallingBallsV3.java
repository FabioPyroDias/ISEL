 ...
 * 
 */
public class C6FallingBallsV3 extends JFrame {
	...
	public void init() {
		...
		buttonsPanel.add(buttonSaveToFile);
			...
			public void mousePressed(MouseEvent e) {
				...
				if (e.getButton() == MouseEvent.BUTTON3) {
					...
					if (numberOfSquares < squares.length) {
						squares[numberOfSquares] = new Square2(myPanel,
								e.getX(), e.getY(),
								new Color(C6FallingBallsV3.rg.nextInt(256),
										C6FallingBallsV3.rg.nextInt(256),
										C6FallingBallsV3.rg.nextInt(256)));
	...
	private void saveToFile() {
		BufferedImage img = new BufferedImage(myPanel.getWidth(),
				myPanel.getHeight(), BufferedImage.TYPE_INT_ARGB);

		Graphics2D g2 = img.createGraphics();

		for (int i = 0; i < numberOfSquares; i++) {
			squares[i].paintSquare(g2);
		}
	...
	public static void main(String[] args) {
		...
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				C6FallingBallsV3 myFrame = new C6FallingBallsV3();
				myFrame.init();
				// life goes on
	...
	class Square2 {
		...
		public Square2(MyPanel4 myPanel, int x, int y, Color color) {
			...
			int delta = 0 + C6FallingBallsV3.rg.nextInt(300);
			System.out.println("Quadrado com timer -> " + delta);
			timer = new Timer(delta, alTimer);

			// start to move the squares, after some initial delay
			// timer.setInitialDelay(100 + C6FallingBallsV3.rg.nextInt(200));
			timer.start();
		}
		...
		public void moveDown() {
			yPos++;
			if (yPos + height > C6FallingBallsV3.this.height) {
				yPos = 0;
				xPos = C6FallingBallsV3.rg
						.nextInt(C6FallingBallsV3.this.width - width);
			}

			if (xPos + width > C6FallingBallsV3.this.width)
				xPos -= 2;
		}
