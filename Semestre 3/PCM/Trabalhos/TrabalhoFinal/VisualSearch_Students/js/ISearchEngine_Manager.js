...
function main() {
    ...
    let keywordBar = document.getElementById("searchBar");

    app.colors.forEach(function(colorID) {
        let color = document.getElementById(colorID);
        
        color.addEventListener('click', function()
        {
            app.searchColor(keywordBar.value.toLowerCase(), colorID, canvas);
        }, false);
    });

    searchButton.addEventListener('click', function()
    {
        app.searchKeywords(keywordBar.value.toLowerCase(), canvas);
    }, false);    
}
